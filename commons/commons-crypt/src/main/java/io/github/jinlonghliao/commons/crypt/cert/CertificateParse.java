...
/**
 * Purpose:自定义证书类，通过BC包的类解析证书，并将解析后的证书信息放到此类属性中，方便获取
 * <p>
 * 此类依赖BC包
 *
 * @since 1.1.0
...
public class CertificateParse {
    private static final Logger log = LoggerFactory.getLogger(CertificateParse.class);
     ...
     * @throws Exception
     */
    public CertificateParse(File certFile) throws Exception {
     ...
     * @throws Exception
     */
    public CertificateParse(String certB64) throws Exception {
     ...
     * @throws Exception
     */
    public CertificateParse(byte[] certEncode) throws Exception {
        init(certEncode);
    }
    ...
    private String getCertItemValue(AttributeTypeAndValue atv) throws Exception {
        ...
        if (value instanceof DERPrintableString) {
            ...
            // log.debug("type:DERPrintableString---" +
            // BCStyle.INSTANCE.oidToDisplayName(type) + "=" + valueStr);
            ...
            // log.debug("type:DERVisibleString---" +
            // BCStyle.INSTANCE.oidToDisplayName(type) + "=" + valueStr);
        } else {
            log.info("type:other---" + BCStyle.INSTANCE.oidToDisplayName(type) + "=" + valueStr);
            // log.debug("type:other---" + BCStyle.INSTANCE.oidToDisplayName(type)
            // + "=" + valueStr);
        }
    ...
    private String getDN(X500Principal x500Principal, ASN1ObjectIdentifier oid) {
        ...
        try {
            ...
            for (int i = 0; i < seq.size(); i++) {
                ...
                /*
                 * DERSet sets = (DERSet)seq.getObjectAt(i); Enumeration<Object> enumer =
                 * sets.getObjects(); while(enumer.hasMoreElements()) { ASN1Sequence curSeq =
                 * (ASN1Sequence)enumer.nextElement(); AttributeTypeAndValue atv =
                 * AttributeTypeAndValue.getInstance(curSeq); DERObjectIdentifier type =
                 * atv.getType();
                 *
                 * if (oid.equals(X509Name.DN_QUALIFIER)) { if (!dnSb.toString().equals("")) {
                 * dnSb.append(", "); } dnSb.append(X509Name.DefaultSymbols.get(type) + "=" +
                 * getCertItemValue(atv)); } else if (type.equals(oid)) {
                 * dnSb.append(getCertItemValue(atv)); break; } }
                 */
            }
        } catch (Exception ex) {
     ...
     *
     * @param dn "C=中国,ST=北京市,L=北京市,O=中安网脉,OU=研发部,CN=SH___CA,E=ca@sis.com"
